...
(function () {
    ...
    class MiniZip {
        ...
        generateBlob() {
            ...
            for (const file of this.files) {
                ...
                dvLfh.setUint16(6, 0x0800, true); // UTF-8 Flag
                ...
                dvCdfh.setUint16(8, 0x0800, true); // UTF-8 Flag
    ...
    style.innerHTML = `
        ...
        body.cycu-iframe-dark-mode, .cycu-iframe-dark-mode #viewerContainer, .cycu-iframe-dark-mode #content-wrapper, .cycu-iframe-dark-mode #outerContainer {
            background-color: #1e293b !important;
        }
        body.cycu-iframe-dark-mode #viewer .page, 
        body.cycu-iframe-dark-mode canvas, 
        body.cycu-iframe-dark-mode .pdfViewer .page,
        body.cycu-iframe-dark-mode .page {
            filter: invert(0.88) hue-rotate(180deg) brightness(0.95) contrast(1.1) !important;
            background-color: #1e293b !important;
        }
...
body.cycu-pdf-hide-native #annotation-view { display: none !important; }

/* 🌓 護眼深色模式：背景反黑與 Canvas / iframe 反轉濾鏡 */
...
body.cycu-pdf-dark-mode .canvasWrapper,
body.cycu-pdf-dark-mode iframe {
    filter: invert(0.9) hue-rotate(180deg) brightness(0.95) contrast(1.05) !important;
    ...
    function makeDraggable(element, handle) {
        ...
        const clampPosition = (left, top) => {
            ...
            return {
                x: Math.max(10, Math.min(left, maxLeft)),
                y: Math.max(10, Math.min(top, maxTop))
            };
        };
        ...
        if (savedLeft !== null && savedTop !== null) {
            ...
            const clamped = clampPosition(parseInt(savedLeft, 10), parseInt(savedTop, 10));
        ...
        window.addEventListener('resize', () => {
            if (element.style.transform === 'none') {
                const currentLeft = parseInt(element.style.left, 10) || 10;
                const currentTop = parseInt(element.style.top, 10) || 10;
        ...
        const onMove = (e) => {
            ...
            const rawLeft = initialLeft + (clientX - startX);
            const rawTop = initialTop + (clientY - startY);
    ...
    setInterval(() => { sendCommandToYTIframes('addEventListener', ['onStateChange']); }, 2500);
            ...
            card.innerHTML = `
                <div style="border-radius:14px; border:1px solid #e2e8f0; box-shadow:0 4px 14px rgba(0,0,0,0.03); background:#ffffff; overflow:hidden;">
                    <div style="background:linear-gradient(135deg, #6366f1, #4f46e5); padding:12px 18px; color:white; display:flex; align-items:center; justify-content:space-between;">
                        ...
                        <span style="font-size:10px; opacity:0.9; background:rgba(255,255,255,0.22); padding:2px 6px; border-radius:10px; font-weight:bold;">v2.1</span>
                    </div>
        ...
        sortedSecNumbers.forEach(secNum => {
            ...
            files.forEach(file => {
                ...
                row.innerHTML = `
                    ...
                    <span style="font-size:12px; color:#1e293b; flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${escapeHtml(file.name)}">${escapeHtml(file.name)}</span>
                `;
            ...
            for (const modname in items) {
                ...
                <div class="section-item">
                    <div class="course-section-header d-flex cycu-accordion-header">
                        <div class="d-flex align-items-center position-relative w-100" style="pointer-events:none;">
                            ...
                            <h3 class="h5 sectionname mb-0 w-100" style="margin-left: 6px;">${escapeHtml(config[modname]?.title || modname)}</h3>
                        </div>
                    </div>
                ...
                for (const week of sortedWeeks) {
                    ...
                    for (const item of weekItemsList) {
                        ...
                        sectionHTML += `<div class="activity-item mb-1"><div class="d-flex align-items-center"><div class="activity-icon me-2">${logoUrl ? `<img src="${logoUrl}" width="18" height="18">` : ''}</div><div class="activityname"><a href="${escapeHtml(item.url)}" class="aalink" style="position:relative; z-index:10; text-decoration: none; font-weight: 500; font-size:13px;">${escapeHtml(item.name)}</a></div></div></div>`;
                    }
                    sectionHTML += `</div></li>`;
                }
            ...
            toast.innerHTML = `
                <div style="width:100%; display:flex; flex-direction:column; gap:6px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-weight:700; font-size:12px; color:#38bdf8; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:240px;" title="${escapeHtml(currentFileName)}">
    ...
    function applyDarkToIframe(isDark) {
        document.querySelectorAll('iframe').forEach(iframe => {
            try {
                const doc = iframe.contentDocument || iframe.contentWindow?.document;
                if (doc) {
                    if (isDark) {
                        doc.body?.classList.add('cycu-iframe-dark-mode');
                        doc.documentElement?.classList.add('cycu-iframe-dark-mode');
                        let darkStyle = doc.getElementById('cycu-iframe-dark-style');
                        if (!darkStyle) {
                            darkStyle = doc.createElement('style');
                            darkStyle.id = 'cycu-iframe-dark-style';
                            darkStyle.innerHTML = `
                                html.cycu-iframe-dark-mode, body.cycu-iframe-dark-mode,
                                .cycu-iframe-dark-mode #viewerContainer, .cycu-iframe-dark-mode #outerContainer, .cycu-iframe-dark-mode #content-wrapper {
                                    background-color: #1e293b !important;
                                }
                                body.cycu-iframe-dark-mode #viewer .page,
                                body.cycu-iframe-dark-mode canvas,
                        ...
                        iframe.classList.add('cycu-styled-dark');
                    } else {
                        doc.body?.classList.remove('cycu-iframe-dark-mode');
                        doc.documentElement?.classList.remove('cycu-iframe-dark-mode');
                        iframe.classList.remove('cycu-styled-dark');
                    }
                }
            } catch (e) {
                iframe.classList.remove('cycu-styled-dark');
    ...
    function toggleIframeNativeToolbar(hide) {
        document.querySelectorAll('iframe').forEach(iframe => {
            try {
                const doc = iframe.contentDocument || iframe.contentWindow?.document;
                if (doc) {
                    let tbStyle = doc.getElementById('cycu-iframe-tb-style');
                    if (!tbStyle) {
                        tbStyle = doc.createElement('style');
                        tbStyle.id = 'cycu-iframe-tb-style';
                        doc.head.appendChild(tbStyle);
                    }
                    tbStyle.innerHTML = hide ? `
                        #toolbarContainer, #toolbarViewer, #pdfannotator-toolbar, .pdfannotator-toolbar, #toolbar, .toolbar {
    ...
    function createPDFSmartAssistant(fullUrl) {
            ...
            <div style="border-radius:14px; border:1px solid #e2e8f0; background:#ffffff; box-shadow:0 4px 16px rgba(0,0,0,0.04); overflow:hidden; width:100%;">
                ...
                <div style="padding:10px 14px; display:flex; flex-direction:column; gap:8px; background:#fafafa;">
                    <div style="display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap:6px;">
                        ...
                        <button id="cycu-pdf-native-toggle" class="cycu-slim-btn cycu-pdf-btn-active">⚙️ 顯示原廠列</button>
        ...
        // 🌟 阻斷整個卡片的冒泡，徹底消除 Moodle handleDocumentClick 報錯
        ...
        btnDark.addEventListener('click', (e) => {
            ...
            updateDarkModeUI();
            showToast(isDarkMode ? "🌓 已啟動護眼深色模式" : "☀️ 已還原預設明亮模式");
        });
        ...
        btnNative.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            const isCurrentlyHidden = document.body.classList.contains('cycu-pdf-hide-native');

            if (isCurrentlyHidden) {
                document.body.classList.remove('cycu-pdf-hide-native');
                btnNative.classList.remove('cycu-pdf-btn-active');
                btnNative.innerHTML = "⚙️ 隱藏原廠列";
                showToast("⚙️ 已顯示原生工具列與側邊抽屜");
            } else {
                document.body.classList.add('cycu-pdf-hide-native');
                btnNative.classList.add('cycu-pdf-btn-active');
                btnNative.innerHTML = "⚙️ 顯示原廠列";
                showToast("⚙️ 已隱藏原生工具列，最大化閱讀空間");
            }
            window.dispatchEvent(new Event('resize'));
        });
        ...
        document.getElementById('cycu-pdf-note-copy').addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            noteArea.select();
        ...
        document.getElementById('cycu-pdf-note-export').addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
        ...
        document.getElementById('cycu-pdf-prev').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); nativeGoPrevPage(); });
        document.getElementById('cycu-pdf-next').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); nativeGoNextPage(); });
        document.getElementById('cycu-pdf-zoom-in').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); nativeZoomIn(); });
        document.getElementById('cycu-pdf-zoom-out').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); nativeZoomOut(); });
        document.getElementById('cycu-pdf-zoom-fit').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); nativeZoomFit(); });
    }
        const savedNoteKey = `cycu_note_${pdfId}`;
        noteArea.value = localStorage.getItem(savedNoteKey) || '';
